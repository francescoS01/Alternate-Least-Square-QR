...
function low_rank_LSQR(A, k, e, V_iterative)
	...
	while dif_UV > e #dif_err > e
		...
		for i in 1:m
			a_col = copy(A'[:, i])
			U_iterative'[:, i] = LS_QR2(copy(Q), copy(R), v_r, v_c, copy(a_col))
		end
		...
		for s in 1:n
			a_col = copy(A[:, s])
			V_iterative'[:, s] = LS_QR2(copy(Q), copy(R), u_r, u_c, copy(a_col))
		end
...
function V_sub(A_sub, Q_U, R_U, u_r, u_c, k)
	...
	for i in 1:n_sub
		a_col = copy(A_sub[:, i])
		# V_iterative'[:, i] = LS_QR(copy(U), copy(a_col))
		V_iterative'[:, i] = LS_QR2(copy(Q_U), copy(R_U), u_r, u_c, copy(a_col))
	end
	return V_iterative'
end
...
function U_sub(A_sub_tr, Q_V, R_V, v_r, v_c, k)
	...
	for i in 1:m_sub
		a_col = copy(A_sub_tr[:, i])
		U_iterative'[:, i] = LS_QR2(copy(Q_V), copy(R_V), v_r, v_c, copy(a_col))
	end
	return U_iterative'
end